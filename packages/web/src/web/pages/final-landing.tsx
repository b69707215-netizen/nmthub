...
import { courses, quizBank, reviews, subjects, type Course } from "../landing-content";
...
export default function FinalLanding() {
  ...
  const easyDone = questions.slice(0, 5).filter((_, itemIndex) => answers[`${subject}-${itemIndex}`] !== undefined).length;
  const hardDone = questions.slice(5).filter((_, itemIndex) => answers[`${subject}-${itemIndex + 5}`] !== undefined).length;
